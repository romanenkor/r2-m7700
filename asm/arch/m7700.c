...
/*
        Reads args from the opcode prefix arrays in the header, based off of
   prefix
...
static OpCode *GET_OPCODE(ut16 instruction, byte prefix) {
  ...
  return (prefix == 0x89
              ? ops89 + instruction
              : (prefix == 0x42 ? ops42 + instruction : ops + instruction));
}

static char *parse_args(OpCode *opcd, RAsmOp *op, unsigned int pc,
                        unsigned int pb, ut8 *buf, int prefix, bool flag_x,
                        bool flag_m, RAsm *a) {
  ...
  case BBCD:
    ...
    if (flag_m || flag_x) { // larger flags asserted
      ...
      varS = read_8(buf, 4);
      snprintf(args, bufsize, "4,#0x%04x,0x%02x,0x%06x\0", read_16(buf, 2),
               read_8(buf, 1),
               pb | ((pc + op->size + varS) & 0xffff)); //, int_8_str(varS));

    } else {         // smaller
      op->size += 3; // maybe change the last to 0x%04hx
      varS = read_8(buf, 3);
      snprintf(args, bufsize, "4,#0x%02x,0x%02x,0x%06x\0", read_8(buf, 2),
               read_8(buf, 1),
               pb | ((pc + op->size + varS) & 0xffff)); //, int_8_str(varS));
    }
    break;
  ...
  case BBCA:
    ...
    if (flag_m || flag_x) { // larger
      op->size += 5;
      varS = read_8(buf, 5);
      snprintf(args, bufsize, "4,#0x%04x,$0x%04x,0x%06x\0", read_16(buf, 3),
               read_16(buf, 1),
               pb | ((pc + op->size + varS) & 0xffff)); //, int_8_str(varS));
    } else {                                            // smaller
      op->size += 4;
      varS = read_8(buf, 4);
      snprintf(args, bufsize, "4,#0x%02x,$0x%04x,0x%06x\0", read_8(buf, 3),
               read_16(buf, 1),
               pb | ((pc + op->size + varS) & 0xffff)); //, int_8_str(varS));
    }
  ...
  case LDM4:                // ldm
    if (flag_m || flag_x) { // larger
      snprintf(args, bufsize, "2,#%04hx,$0x%02x\0", read_16(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 3;
    } else { // smaller
      snprintf(args, bufsize, "2,#%02x,$0x%02x\0", read_8(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 2;
    }
    break;
  ...
  case LDM5:                // ldm long
    if (flag_m || flag_x) { // larger
      snprintf(args, bufsize, "2,#%04hx,$0x%04hx\0", read_16(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 4;
    }

    else { // smaller
      snprintf(args, bufsize, "2,#%02x,$0x%04hx\0", read_8(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 3;
    }
    break;
  ...
  case LDM4X:               // ldm X direct
    if (flag_m || flag_x) { // larger
      snprintf(args, bufsize, "3,#$%04hx,$%02x,xl\0",
               read_16(buf, op->size + 1), read_8(buf, op->size));
      op->size += 3;
    } else { // smaller
      snprintf(args, bufsize, "3,#$%02x,$%02x,xl\0", read_8(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 2;
    }
    break;
  ...
  case LDM5X:               // ldm x direct long
    if (flag_m || flag_x) { // larger
      snprintf(args, bufsize, "3,#$%04hx,$%04x,xl\0",
               read_16(buf, op->size + 2), read_16(buf, op->size));
      op->size += 4;
    } else { // smaller
      snprintf(args, bufsize, "3,#$%02x,$04hx,xl\0", read_8(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 3;
    }
    break;
  ...
  case AL:
    snprintf(args, bufsize, "1,$%06hx\0",
  ...
  case MVN:
  case MVP:
    snprintf(args, bufsize, "2,$%02x,$%02x\0", read_8(buf, op->size + 1),
             read_8(buf, op->size));
    op->size += 2;
    break;
  default:
...
static int m7700_disassemble(RAsm *a, RAsmOp *op, ut8 *buf, ut64 len) {
  ...
  int prefix = 0;
  instruction =
      read_8(buf, 0); // grab instruction from buffer, with offset of 0
  ...
  case 0x42: // x42 prefix -
    ...
    instruction =
        read_8(buf, 1); // grab next instruction from buffer, with offset of 1
  ...
  strcat(opname, "\0");
  r_strbuf_set(&op->buf_asm, opname);

  // X_FLAGS[a->pc]
  //  parse all variables, tokenize them, parse
  char *vars = strtok(parse_args(opcd, op, (a->pc & 0xffff), a->pc >> 16, buf,
                                 prefix, !(GLOB_X) && (opcd->flag == X),
                                 !(GLOB_M) && opcd->flag == M, a),
                      ",");
  ...
  //   if (strcmp(opname, "JSR")){ // attempt to define the boundaries for the
  //   JSR
  ...
  if (*arg) {
    ...
    r_strbuf_append(&op->buf_asm, " --  m:");
    r_strbuf_append(&op->buf_asm, M_FLAGS[a->pc] ? "1" : "0");
    r_strbuf_append(&op->buf_asm, " x:");
    r_strbuf_append(&op->buf_asm, X_FLAGS[a->pc] ? "1" : "0");
    r_strbuf_append(&op->buf_asm, "\0");
  }
