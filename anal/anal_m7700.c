...
static bool ANAL_M_FLAGS[0xFFFF]; // by default, these should be treated as 0 =
                                  // 1 and 1 = 0
...
static char *parse_anal_args(OpCode *opcd, RAnalOp *op, unsigned int pc,
                             unsigned int pb, const unsigned char *buf,
                             int prefix, bool flag_x, bool flag_m, RAnal *a,
                             ut64 addr) {
  ...
  case IMP: // implied addressing mode - single instruction addressed to int.
            // register
    snprintf(args, 60, "0");
    break;
  ...
  case RELB: // Relative addr
    ...
    snprintf(
        args, 60, "1,0x%02hx",
        pb | ((addr + op->size + varS) &
              0xffff)); //, int_8_str(varS)); // Need to add a way to parse the
                        // param from the instruction in buff for last param
    break;
  ...
  case PER:
    ...
    snprintf(
        args, 60, "1,0x%04x",
        pb | ((addr + op->size + var) &
              0xffff)); // int_16_str(var)); // Need to add a way to parse the
                        // param from the instruction in buff for last param
    op->size += 2;
    break;

  case IMM: // immediate addressing - format: acc val
    // check addressing mode - first is for 8 bit addressing mode, second for 16
    // bit
  ...
  case BBCD:
    // check addressing mode - first is for 16 bit addressing mode, second for 8
    // bit
    if (flag_m || flag_x) { // larger instruction
      op->size += 4;        // maybe change the last to 0x%04hx
      varS = read_8(buf, 4);
      snprintf(args, 60, "3,0x%04x,%d,0x%06x\0", read_16(buf, 2),
               read_8(buf, 1), pb | ((addr + op->size + varS) & 0xffff));
    } else {         // smaller mem size flags asserted
      op->size += 3; // maybe change the last to 0x%04hx
      varS = read_8(buf, 3);
      snprintf(args, 60, "3,0x%02x,%d,0x%06x\0", read_8(buf, 2), read_8(buf, 1),
               pb | ((addr + op->size + varS) & 0xffff));
    }
    break;
  ...
  case BBCA:
    ...
    if (flag_m || flag_x) { // larger
      ...
      varS = read_8(buf, 5);
      snprintf(args, 60, "3,0x%04x,0x%04x,0x%06x\0", read_16(buf, 3),
               read_16(buf, 1), pb | ((addr + op->size + varS) & 0xffff));
    } else { // smaller
      op->size += 4;

      varS = read_8(buf, 4);
      snprintf(args, 60, "3,0x%02x,0x%04x,0x%06x\0", read_8(buf, 3),
               read_16(buf, 1), pb | ((addr + op->size + varS) & 0xffff));
    }
    break;
  ...
  case LDM4:
    if (flag_m || flag_x) { // larger
      snprintf(args, 60, "2,0x%04hx,0x%02x\0", read_16(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 3;
    } else { // smaller
      snprintf(args, 60, "2,0x%02x,0x%02x\0", read_8(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 2;
    }
    break;
  ...
  case LDM5:
    if (flag_m || flag_x) { // larger
      snprintf(args, 60, "2,0x%04hx,0x%04hx\0", read_16(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 4;
    } else { // smaller
      snprintf(args, 60, "2,0x%02x,0x%04hx\0", read_8(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 3;
    }
    break;
  ...
  case LDM4X:
    if (flag_m || flag_x) { // larger
      snprintf(args, 60, "3,0x%04hx,0x%02x,xl\0", read_16(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 3;
    } else { // smaller
      snprintf(args, 60, "3,0x%02x,0x%02x,xl\0", read_8(buf, op->size + 1),
               read_8(buf, op->size));
      op->size += 2;
    }
    break;
  case LDM5X:
    if (flag_m || flag_x) { // larger
      snprintf(args, 60, "3,0x%04x,0x%04x,xl\0", read_16(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 4;
    } else { // smaller
      snprintf(args, 60, "3,0x%02x,0x%04x,xl\0", read_8(buf, op->size + 2),
               read_16(buf, op->size));
      op->size += 3;
    }
    break;
  ...
  case AL:
    snprintf(args, 60, "1,0x%06hx\0",
             read_24(buf, op->size)); // might need to be set to 06x
    op->size += 3;
    break;
 ...
 */
static char *calc_addressing(RAnal *anal, RAnalOp *op, OpCode *opcd, char *addr,
                             char *param) {

  char *ret = (char *)malloc(20); // allocate return characters

  // First - calculate addresses based off addressing type. Put in any initial
  // flags
  switch (opcd->arg) {

  case IMP: // implied addressing mode - single instruction addressed to int.
            // register nothing to be done here
    break;
  ...
  case IMM: // immediate addressing - format: acc val
            // check addressing mode - first is for 8 bit addressing mode,
            // second for 16 bit
  ...
  case DI: // direct indirect addressing mode
    ...
    sprintf(ret,
            "[%s,dpr,+],dt,=", addr); // indirectly read from mem, store in DT
  ...
  case DIY: // direct indirect indexed Y addressing mode
    sprintf(ret, "[%s,dpr,+],y,+,dt,=[2]",
            addr); // indirectly read from mem using x + dpr + op, store in DT
    op->type |= R_ANAL_OP_TYPE_IND;
    break;
  case DLI: // direct indirect long addressing mode
    sprintf(ret, "[%s,dpr,+],dt,=[3]",
            addr); // indirectly read from mem using dpr + op, store in DT
    op->type |= R_ANAL_OP_TYPE_IND;
    break;
  case DLIY: // direct indirect long indexed Y addressing mode
    sprintf(
        ret, "[%s,dpr,+],y,+=[3],dt,=",
        addr); // indirectly read from mem using dpr + op, add to y, store in DT
    op->type |= R_ANAL_OP_TYPE_IND;
    break;
...
static int m7700_anal_op(RAnal *anal, RAnalOp *op, ut64 addr, const ut8 *data,
                         int len) {
  ...
  int prefix = 0;
  ut8 instruction =
      read_8(data, 0); // grab the instruction from the r_asm method
  ...
  // pull the prefix of the instruction off, grabing from the tables
  // corresponding to the addressing mode
  switch (instruction) {
  // first two cases, remove prefix - otherwise just pass instruction
  case 0x42: // x42 prefix -
    instruction = read_8(
        data, op->size); // grab next instruction from buffer, with offset of 1
  ...
  switch (opcd->op) { // check the flags and potential manipulation instructions
                      // before evaluating opcode/mem length
  ...
  char *vars =
      parse_anal_args(opcd, op, op->addr & 0xffff, op->addr >> 16, data, prefix,
                      !(ANAL_GLOB_X) && (opcd->flag == X),
                      !(ANAL_GLOB_M) && (opcd->flag == M), anal, op->addr);
  ...
  case LDY: // load to index reg Y
    switch (opcd->arg) {
      ...
      r_strbuf_setf(&op->esil, "%s,[],%s,=", ops[2],
                    ops[1]); // LOAD ACC FROM MEM
  ...
  case LDM: // load to memory (immediate)
    ...
    case LDM4X:
      ...
      op->type = R_ANAL_OP_TYPE_LOAD;
      op->ptr = r_num_get(NULL, (const char *)ops[2]) +
                r_anal_esil_reg_read(&op->esil, "xl", NULL, NULL);
      break;
    default:
      r_strbuf_setf(&op->esil, "%s,%s,[],=", ops[1], ops[2]);
      op->type = R_ANAL_OP_TYPE_LOAD;
  ...
  case CMPB:
    ...
    case AX:
    case IMM:
      r_strbuf_setf(&op->esil, "%s,%s,-,$c4,cf,=,$z,zf,=,$n,nf,=", ops[2],
                    ops[1]);
      // op->ptr = ops[2];
  ...
  case MPYS: // multiply (SIGNED)
    ...
    case AX:
    case IMM:
      r_strbuf_setf(&op->esil, "%s,%s,*,%s,=,$z,zf,=,$n,nf,=,$o,o,=", ops[1],
                    ops[2], ops[1]);
  ...
  case INC: // increment mem by 1
    r_strbuf_setf(&op->esil, "%s,[],++,=,$z,zf,=,$n,nf,=,$o,of,=",
                  ops[1]); // add 1 to val at mem
    op->type = R_ANAL_OP_TYPE_ADD | R_ANAL_OP_TYPE_IND;
    op->ptr = r_num_get(NULL, (const char *)ops[1]);
    break;
  ...
  case EORB: // mem and accumulator B
    ...
    default:
      ...
      op->ptr = ops[2];
      r_strbuf_setf(&op->esil, "%s,%s,[],^,%s,=,$z,zf,=", ops[1], ops[2],
                    ops[1]);
      break;
    }
    break;
  ...
  case LSR:
    r_strbuf_setf(&op->esil, "%s,1,<<,%s,=", ops[1], ops[1]);
    op->type = R_ANAL_OP_TYPE_SHR; // I don't know what op type to use for this,
                                   // so placeholder
    break;
  ...
  case BRA: // branch ALWAYS
    ...
    r_strbuf_setf(&op->esil, "%s,pc,=",
                  ops[1]); //"%d,pc,+=,%s,pc,=", op->size, ops[1]);// set stack
                           // ptr 	printf("BRA value: %s", ops[1]);
    break;
  ...
  case BBC: // branch on bit clear
    ...
    r_strbuf_setf(&op->esil, "%s,%s,[],&=,!,?{,2,s,-=,pc,s,=[2],%s,pc,=,}",
                  ops[1], ops[2], ops[3]); // set stack ptr
    break;

  case BBS: // branch on bit set
    op->type = R_ANAL_OP_TYPE_CJMP;
    op->jump = r_num_get(NULL, (const char *)ops[3]); // grab op conditional
    r_strbuf_setf(&op->esil, "%s,&=,%s,?{,2,s,-=,pc,s,=[2],%s,pc,=,}", ops[1],
                  ops[2], ops[3]); // set stack ptr
  ...
  case BCC:                         // branch on carry clear
    ...
    op->jump = r_num_get(
        NULL, (const char *)ops[1]); // r_num_get (NULL, (const char *)ops[1]);;
                                     // // grab op conditional
  ...
  case BNE:                         // branch if zero flag clear
    ...
    op->jump = r_num_get(
        NULL, (const char *)ops[1]); // r_num_get (NULL, (const char *)ops[1]);;
                                     // // grab op conditional
    op->fail = op->addr + op->size;
    r_strbuf_setf(&op->esil, "zf,!,?{,2,s,-=,pc,s,=[2],%s,pc,=,}",
                  ops[1]); // push PC to stack
    break;
  ...
  case BPL: // branch if negative flag clear
    ...
    r_strbuf_setf(&op->esil, "nf,!,?{,2,s,-=,pc,s,=[2],%s,pc,=,}",
                  ops[1]); // push PC to stack
    break;
  case BMI: // branch if negative flag set -- have to change implementation of
            // neg flag
  ...
  case BVC:                         // branch if overflow flag clear
    ...
    op->jump = r_num_get(
        NULL, (const char *)ops[1]); // r_num_get (NULL, (const char *)ops[1]);;
                                     // // grab op conditional
    op->fail = op->addr + op->size;
    r_strbuf_setf(&op->esil, "of,!,?{,2,s,-=,pc,s,=[2],%s,pc,=,}",
                  ops[1]); // push PC to stack
    break;
  ...
  case JSR: // save current address in stack, jump to subroutine
    ...
    r_strbuf_setf(&op->esil,
                  "$2,s,-=,pc,s,=[2],%s,pc,=", ops[1]); // push PC to stack
    op->fail = op->addr + op->size;
  ...
  case RTI: // return from interrupt
    ...
    r_strbuf_setf(&op->esil, "s,[],pc,=,2,s,+=");
    op->eob = true;
  ...
  case RTL: // return from subroutine long, restore program bank contents
    op->type = R_ANAL_OP_TYPE_RET;
    op->eob = true;
    r_strbuf_setf(&op->esil, "s,[2],pc,=,2,s,+=");
  ...
  case BRK: // execute software interrupt
	...
	//			%s,pc,=",
            //			op->size, ops[1]
    //		); // avoid the ESIL For this for now, it's not in the binary
    // I'm analyzing
    // the above ESIL does the following
    // - sets the ID flag bit to 1, disabling interrupts
    // - takes the program address size, and the PC, adds them together, then
    // loads it to the stack
...
static int set_reg_profile_7700(RAnal *anal) {
  const char *p =
      ...
      "gpr	pg	.8  0	0\n" // program bank register, highest 8 bits of
                                     // PC
      "gpr	s	.16 24	0\n" // stack pointer
      "gpr	ax	.16 40	0\n" // accumulator A
      //"gpr    ah      .8  8  0\n"  // high 8 bits of A - remains unchanged
      // when M flag set
      "gpr	al	.8  40	0\n" // low 8 bits of A
      "gpr	bx	.16 56	0\n" // accumulator B
      //"gpr    bh      .8  8  0\n"  // high 8 bits of B
      "gpr	bl	.8  56	0\n" // low 8 bits of B
      "gpr	x	.16  72	0\n" // index register X
      "gpr	xl	.8  72	0\n" // low bits for index register X - active
                                     // when X flag set
      "gpr	y	.16  88	0\n" // index register Y
      "gpr	yl	.8  88	0\n" // low bits for index register Y - active
                                     // when X flag set
      "gpr	db	.8	96	0\n" // data bank register
      ...
      "flg	ix	.1	123	0\n" // index register length flag - bit
                                             // 4
...
static int esil_m7700_init(RAnalEsil *esil) { return true; }

static int esil_m7700_fini(RAnalEsil *esil) { return true; }
